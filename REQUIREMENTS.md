...
|---|---|---|---|---|
| 14 | 2026-09-23 | FR-045 | The line naming the program and its address prints once, above the title; the copy below the last event goes. The printed sheet leaves no page margin, so the browser has nowhere to draw its own header and footer. | Owner's ruling after a real test print. The letterhead at the top already says what produced the sheet, so repeating it at the foot said nothing twice. The print also carried the date, the window title, `wails.localhost` and a page number, none of which are ours: Chromium draws those inside the page margin and a page can reach them no other way, so `@page { margin: 0 }` removes the room they print in and the margin the paper needs moves onto the sheet. The trade is stated rather than hidden: a record running past one page begins the second at the physical edge, where a printer cannot mark. Unverified here, since nothing in the development sandbox prints; only a real print settles it. |
