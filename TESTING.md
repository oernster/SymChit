...
| The browser prints no furniture of its own: no date, no window title, no `wails.localhost`, no page number. | **Found wrong on paper 2026-09-23, fix written, UNVERIFIED.** The test print carried all four. Chromium draws them inside the page margin, so `@page { margin: 0 }` in the print stylesheet leaves them nowhere to print and the paper's own margin moves onto the sheet. Nothing in the development sandbox prints and the browser pane cannot show print furniture, so only a real print settles it. The same print settles the trade it makes: a record long enough to run to a second page begins that page at the physical edge, where a printer cannot mark, so its first line can be lost. If the trade is not wanted, the alternative is to leave the margins alone and untick "Headers and footers" in the Windows print dialog, which sticks per user. |
