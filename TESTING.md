...
| The browser's own furniture (the date, the window title, `wails.localhost`, the page number) stays off the paper. | Measured on paper 2026-09-23: with no page margin it does not print. A browser draws it inside the page margin and can reach nowhere else, so leaving no margin is what removes it. Owed on the Flatpak and the disk image, which print through a different engine. |
