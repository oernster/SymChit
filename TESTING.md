...
| The framing prints: the program and its address above the title, with the statement under the range. | Measured on paper 2026-09-23: both printed as intended, with the groups, the severities and the notes. The closing copy of the address has gone since (Amendment 14), so the reading that is now owed is that the sheet carries the line once, at the top; nowhere else. |
| A record that runs past one page prints correctly on every page. | **Found broken on paper 2026-09-23, then fixed and measured against the print engine.** The first answer, `@page { margin: 0 }` with padding standing in for the margin, removed the browser's furniture and sliced the first line of page two: padding is applied once to the element, not once per sheet. The sheet is now laid out as a table whose empty head and foot a print engine repeats on every page. Measured by printing a ten page record to PDF through the engine WebView2 runs and reading back the text positions: the first baseline sits 17.7mm to 18.3mm from the top of every page and the last no closer than 17.2mm to the foot. Owed on paper: the same on a real printer, on Windows; also on the Flatpak and the disk image, which print through two other engines. |
| Every printed page says which page it is; the browser's own furniture (the date, the window title, `wails.localhost`) stays off the paper. | Measured 2026-09-23 through the engine WebView2 runs, with the browser's own header and footer switched ON at the protocol rather than suppressed at the command line, which is what a reader with the box ticked in the print dialog gets. A ten page record printed `Page 1 of 10` through `Page 10 of 10`, one to a page, carrying none of the browser's own date, address or page count: declaring an @page margin box is what suppresses them. The paper was US Letter, so the sheet is not relying on A4. Owed on a real printer, plus the Flatpak and the disk image, which print through an engine that may not draw margin boxes at all. |
...
| The printed sheet carries the mark beside the address at the top, then prints it. | Measured on paper 2026-09-23: the mark printed. The mark is covered in the suite, where exactly one appears and it is in the first line; whether a printer draws it, plus how a black and white copier treats it, is a reading only a print can give. |
