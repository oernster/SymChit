...
// FR-065: a run leaves a log. A crash ends the process that has it, so
...
func startLine(when time.Time) string {
	return product.Name + " started " + when.Format(stampLayout) + "\n"
}
...
func TestTheLogIsMadeWithItsFolder(t *testing.T) {
	t.Parallel()
	path := filepath.Join(t.TempDir(), "Local", product.Name, FileName)
	openAndClose(t, path, started)
...
func TestTheLogSitsInTheProductsDataFolder(t *testing.T) {
	...
	got, err := Path()
	if want := filepath.Join(base, product.Name, FileName); err != nil || got != want {
